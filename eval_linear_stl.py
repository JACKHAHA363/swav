...
def main():
    ...
    train_dataset = datasets.STL10(args.data_path, split='train', transform=train_transform)
    val_dataset = datasets.STL10(args.data_path, split='test', transform=val_transform)
    ...
    best_acc = 0
    tb_writer = SummaryWriter(os.path.join(args.dump_path, 'tb'))
    for epoch in range(args.epochs):
        ...
        tb_writer.add_scalar('train/loss', scores[1], epoch)
        tb_writer.add_scalar('train/top1', scores[2], epoch)
        tb_writer.add_scalar('train/top5', scores[3], epoch)
        tb_writer.add_scalar('val/loss', scores_val[0], epoch)
        tb_writer.add_scalar('val/top1', scores_val[1], epoch)
        tb_writer.add_scalar('val/top5', scores_val[2], epoch)
        scheduler.step()
